...
// PAYROLL_CORE_STABLE_V1_POSTGRES — the frozen surface, as a checksum manifest.
//
// DB-M1 (2026-09): the SAME 48 files as PAYROLL_CORE_STABLE_V1. Business logic is
// unchanged; the 28 DB-coupled files and the 16 suites were re-baselined for the
// SQLite -> PostgreSQL persistence migration, only after the Golden Payroll Parity
// Harness proved BUSINESS OUTPUT DIFF = ZERO. The 4 pure formula files are
// byte-identical to the original lock, which is preserved read-only in
// docs/freeze-history/ and is still enforced by `npm run check:payroll-pure`.
//
...
const SHARED_FILES = [
  'database/db.js',                 // canonical PostgreSQL access layer (DB-M1)
  'database/init-db.js',
  'database/seed.js',
...
const FREEZE_VERSION = 'PAYROLL_CORE_STABLE_V1_POSTGRES';

module.exports = { FREEZE_VERSION, FROZEN_FILES, SHARED_FILES, PAYROLL_SUITES, REGRESSION_BASELINE };
